...
class FakeListingApiTest extends TestCase
{
	public function testFilterByMinPrice(): void
	{
		...
		self::assertCount(1, $fakeListings->list(SearchListingsDTO::create()->setMinPrice(800))->getItems());
		self::assertCount(1, $fakeListings->points(SearchListingsDTO::create()->setMinPrice(800))->getFeatures());

		// Fake service has two items with min prices 750 and 1000. We check full items presents
		self::assertCount(2, $fakeListings->list(SearchListingsDTO::create()->setMinPrice(750))->getItems());
		self::assertCount(2, $fakeListings->points(SearchListingsDTO::create()->setMinPrice(750))->getFeatures());
